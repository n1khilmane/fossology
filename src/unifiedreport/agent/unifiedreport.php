...
class UnifiedReport extends Agent
{
  ...
  private function writeReport($contents, $uploadId, $groupId, $userId)
  {
    ...
    $rightColStyleBlue = array("size" => 11, "color" => "0000A0", 'underline' => 'single');
    if ($contents['otherStatement']['includeDNU']) {
      $section->addBookmark('DNUBookmark');
      $bookMarkCell->addLink('DNUBookmark', htmlspecialchars(' NOTE: DO NOT USE files found! Please check Do not use files section', ENT_COMPAT, 'UTF-8'), $rightColStyleBlue, "pStyle", true);
    }
    $titleSubHeadingIrre = "(Path, Files, Licenses)";
